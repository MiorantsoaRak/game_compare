...
-- Started on 2017-03-01 23:12:29
...
COPY carte_graphique (idgraphique, constructeur, model, memoirededie, vrready, vram) FROM stdin;
\.
...
COPY configuration (idconf, processeur, ram, stockage, resolution, os, cartegraphique) FROM stdin;
1  Core i5-4590 3.3GHz / FX-8350 16 15 1336*720 Windows 7   GeForce GTX 970 / Radeon R9 290
\.
...
COPY jeuxvideo (idjeux, idtype, idmode, nomdujeux, datesortie, editeur, prixlancement, restriction_age, tailledujeux, idconf_min, idconf_max) FROM stdin;
3  4  1  Resident evil 7   2017-01-24  Capcom   30 18 15 2  1
\.
...
COPY mode_de_jeux (idmode, libelle) FROM stdin;
1  Solo
2  Multi
\.
...
COPY plateforme (idjeux, idplateforme) FROM stdin;
\.
...
COPY plateforme_de_jeux (idplateforme, libelle) FROM stdin;
1  Xbox 360
2  PS3
3  PS4
4  PS4 Pro
...
COPY soustype (idmere, idtype) FROM stdin;
\.
...
COPY utilisateur (idutilisateur, nom, prenom, datenaissance, email, username, password) FROM stdin;
\.
...
-- Completed on 2017-03-01 23:12:30
