drop index carte_graphique_pk;

drop table carte_graphique;

drop index asso_config_graphique_fk;

drop index configuration_pk;

drop table configuration;

drop index mode_fk;

drop index plateforme_fk;

drop index assoc_type_jeux_fk;

drop index jeuxvideo_pk;

drop table jeuxvideo;

drop index mode_de_jeux_pk;

drop table mode_de_jeux;

drop index plateforme_de_jeux_pk;

drop table plateforme_de_jeux;

drop index sous_type2_fk;

drop index sous_type_fk;

drop index sous_type_pk;

drop table soustype;

drop index typejeux_pk;

drop table typejeux;

drop index utilisateur_pk;

drop table utilisateur;